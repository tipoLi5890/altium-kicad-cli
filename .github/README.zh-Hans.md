...
<p><a href="../README.md">English</a> · <a href="README.zh-Hant.md">繁體中文</a> · <strong>简体中文</strong></p>
...
**当前已提供（v0.13.x）：** KiCad 写入／绘制（22 种 op + 10 种宏，含层次 `add_sheet`、net-diff
...
**前瞻（→ v1.0）：** 契约冻结审计（contract freeze audit）。*按决策暂缓：* 为原理图 PR 把关的 GitHub Action、
`view` 波形面板，以及原生 MCP 服务器（目前代理直接驱动 CLI）。完整计划与验收条件见
