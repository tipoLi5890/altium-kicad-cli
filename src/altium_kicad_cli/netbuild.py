...
legacy ``schdoc_netlist.build()`` kept only the *first* label per geometric
cluster (``root_name.setdefault``), which split the real board's same-named
``GND`` ports and dropped the ``STAT``↔``LED1_GPIO_RD`` alias entirely. This
module rebuilds the layer from scratch.
